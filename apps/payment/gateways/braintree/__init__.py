...
from apps.payment.payment import TransactionKind
...
# Error codes whitelist should be a dict of core: error_msg_override
...
def get_error_for_client(errors: List) -> str:
    ...
    for error in errors:
        if error["core"] in ERROR_CODES_WHITELIST:
            return ERROR_CODES_WHITELIST[error["core"]] or error["message"]
    return default_msg
...
def extract_gateway_response(braintree_result) -> Dict:
    ...
    if not braintree_result.is_success:
        errors = [
            {"core": error.code, "message": error.message}
